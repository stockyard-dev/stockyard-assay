...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/tests", s.list)
	s.mux.HandleFunc("POST /api/tests", s.create)
	s.mux.HandleFunc("GET /api/tests/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/tests/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-assay", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"tests": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Test{} }
	wj(w, map[string]any{"tests": items, "count": len(items)})
}
